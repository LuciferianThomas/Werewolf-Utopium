...
// TESTING ONLY /app/newcommands/

client.newCommands = new Discord.Collection()
const newCommandFiles = fs.readdirSync('./newcommands').filter(file => file.endsWith('.js'))
for (const file of commandFiles) {
...
client.on('message', async message => {
  ...
  const prefix = "u?", // || guild.prefix || config.defaultPrefix,
        mention = `<@${client.user.id}> `,
        mention1 = `<@!${client.user.id}> `
  ...
  if (msg.startsWith(prefix) || msg.startsWith(mention) || msg.startsWith(mention1)) {
    ...
    if (msg.startsWith(prefix)) {
      ...
      shared.prefix = mention1
		...
		shared.commandName = commandName
		const command = client.newCommands.get(commandName) || client.newCommands.find((cmd) => cmd.aliases && cmd.aliases.includes(commandName))

		if (!command) return;
    
    let args = {}
    for (let i = 0; i < command.args.length; i++) {
      if (content.length == 0) return message.channel.send(fn.embed(client, command.args[i].))
    ...
    shared.embedColor = config.embedColor
		...
		try {
			await command.run({client: client, message: message, shared: shared}, args)
		} catch (error) {
			console.log(error)
		}
