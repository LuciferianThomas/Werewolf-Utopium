...
module.exports = {
  ...
  run: async (client, message, args) => {
    ...
    if (moment(player.lastDaily).add(20, "h") >= moment()) {
      ...
      let rdmmsgs = [
        `${message.channel.type == "dm" ? "Y" : `<@${message.author.id}>, y`}ou cannot collect daily rewards for another **${Math.floor(
          diff / 60 / 60
        ) % 24}h ${Math.floor(diff / 60) % 60}m ${diff % 60}s**.`,
        `${message.channel.type == "dm" ? "H" : `<@${message.author.id}>, h`}mm... You need to wait for another **${Math.floor(diff / 60 / 60) %
          24}h ${Math.floor(diff / 60) % 60}m ${diff %
          60}s** to claim your next daily reward!`
      ]
      return await message.channel.send(
        Math.floor(diffclaim / 60 / 60) % 24 < 12
          ? `${message.channel.type == "dm" ? "H" : `<@${message.author.id}>, h`}ow is it "daily" if you claim it **${Math.floor(
              diffclaim / 60 / 60
