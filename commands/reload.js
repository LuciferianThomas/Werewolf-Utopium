...
module.exports = {
 ...
 // botStaffOnly: true,
    ...
    if(command.startsWith("/")){
      ...
      delete require.cache[require.resolve(`/home/sd/wwou/commands/${commandfile.name}.js`)]

      if(command === "shop") delete require.cache[require.resolve(`/home/sd/wwou/util/shop.js`)]
      if(command === "roles") delete require.cache[require.resolve(`/home/sd/wwou/util/roles.js.js`)]
      if (["coins","custom","games","items","logs","roses","talisman","use"].includes(command)) {
        let filesInFolder = fs.readdirSync(`/home/sd/wwou/commands/${command}`).filter(file => file.endsWith('.js'))
        filesInFolder.forEach(file => delete require.cache[require.resolve(`/home/sd/wwou/commands/${command}/${file}`)])
      }

      let props = require(`/home/sd/wwou/commands/${commandfile.name}`);
