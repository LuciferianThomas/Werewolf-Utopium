  ...
  run: async (client, message, args, shared) => {
    ...
    await message.channel.send(
      new Discord.MessageEmbed()
        ...
        .setDescription(
          `Successfully given ${input} ${fn.getEmoji(client, "Coin")} to ${nicknames.get(targetPlayer.id)}${
    ...
    await targetPlayer.send(
      new Discord.MessageEmbed()
        .setTitle("Moar coins!")
        .setDescription(
          `${nicknames.get(message.author.id)} added ${input} ${fn.getEmoji(client, "Coin")} to your balance${
          reason.length ? ` for **${reason}**` : ""}.\n` +
    ...
    await client.channels.cache.get("686761420271910940").send(
      new Discord.MessageEmbed()
        .setTitle("Moar coins!")
        .setDescription(
          `${nicknames.get(message.author.id)} added ${input} ${fn.getEmoji(client, "Coin")} to ${nicknames.get(targetPlayer.id)}'s balance${
          reason.length ? ` for **${reason}**` : ""}.\n` +
