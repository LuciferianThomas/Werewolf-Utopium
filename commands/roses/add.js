...
module.exports = {
  ...
  run: async (client, message, args, shared) => {
    if (!client.guilds.cache.get("522638136635817986").members.cache.get(message.author.id).roles.cache.find(r => r.name.includes("Moderator")))
      return await message.channel.send("You do not have permissions to add roses!")
    ...
    await message.channel.send(
      new Discord.MessageEmbed()
        ...
        .setDescription(
          `Successfully given ${input} ${fn.getEmoji(client, "Rose")} to ${nicknames.get(targetPlayer.id)}${
    ...
    await targetPlayer.send(
      new Discord.MessageEmbed()
        .setTitle("Moar roses!")
        .setDescription(
          `You were given ${input} ${fn.getEmoji(client, "Rose")}${
          reason.length ? ` for **${reason}**` : ""}.\n` +
    ...
    await client.channels.cache.get("690080603634532433").send(
      new Discord.MessageEmbed()
        .setTitle("Moar roses!")
        .setDescription(
          `${nicknames.get(targetPlayer.id)} was given ${input} ${fn.getEmoji(client, "Rose")}${
          reason.length ? ` for **${reason}**` : ""}.\n` +
