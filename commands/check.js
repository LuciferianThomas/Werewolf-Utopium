...
module.exports = {
  ...
  run: async (client, message, args, shared) => {
    ...
    if (
      !["Seer", "Aura Seer", "Spirit Seer", "Sorcerer", "Wolf Seer"].includes(
        gamePlayer.role
      )
    )
      return await message.author.send(
        "You do not have the abilities to check on a player."
      )
    if (!gamePlayer.alive)
      return await message.author.send(
        "You are dead. You can no longer check on a player."
      )
    ...
    if (game.currentPhase % 3 != 0)
      return await message.author.send(
        "You can only check on a player at night."
      )
    ...
    if (gamePlayer.role == "Spirit Seer") {
      ...
      if (gamePlayer.role == "Aura Seer") {
        let embed = new Discord.RichEmbed()
          ...
          .setThumbnail(
            fn.getEmoji(
              client,
              `${
                game.players.find(pl => pl.enchant == targetPlayer.number)
                  ? "Evil"
          ...
          .setDescription(
            `**${target} ${nicknames.get(targetPlayer.id)}** has a${
              (game.players.find(pl => pl.enchant == targetPlayer.number)
                ...
                : targetPlayer.disguised
                ? "Unknown"
                : roles[targetPlayer.role].aura) == "Good"
                ? ""
                : "n"
            } ${
              game.players.find(pl => pl.enchant == targetPlayer.number)
        ...
        if (
          roles[targetPlayer.role].aura == "Unknown" ||
          targetPlayer.disguised
        )
        ...
        if (gamePlayer.role == "Wolf Seer")
          fn.broadcastTo(
            ...
            `The wolf seer checked **${target} ${nicknames.get(
              ...
              client,
              targetPlayer.disguised ? "Illusionist" : targetPlayer.role
            )}**.`
          )
      }
