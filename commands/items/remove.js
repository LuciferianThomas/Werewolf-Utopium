...
module.exports = {
  ...
  run: async (client, message, args, shared) => {
    if (!client.guilds.cache.get("522638136635817986").members.cache.get(message.author.id).roles.cache.find(r => r.name.includes("Moderator")))
      return await message.channel.send("You do not have permissions to remove items!")
    ...
    players.subtract(`${targetPlayer.id}.inventory.${item.itemid}`, am)
    fn.addLog("items", `${message.author.tag} removed ${am} ${item.name}(s) to ${nicknames.get(targetPlayer.id)}, leaving them with a total of ${players.get(`${targetPlayer.id}.inventory.${item.itemid}`)} ${item.name}(s).`)
    ...
    await message.channel.send(
      new Discord.MessageEmbed()
        ...
        .setDescription(
          `Successfully removed ${am} ${item.name}(s) from ${nicknames.get(targetPlayer.id)}\n` +
    ...
    await targetPlayer.send(
      new Discord.MessageEmbed()
        .setTitle("Removed item!")
        .setDescription(
          `${nicknames.get(message.author.id)} removed ${am} ${item.name}(s) from your inventory\n` +
    ...
    await client.channels.cache.get("694656469232123965").send(
      new Discord.MessageEmbed()
        .setTitle("Removed item!")
        .setDescription(
          `${nicknames.get(message.author.id)} removed ${am} ${item.name}(s) from ${nicknames.get(targetPlayer.id)}'s inventory\n` +
