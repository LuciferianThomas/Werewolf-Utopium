...
module.exports = {
  ...
  run: async (client, message, args, shared) => {
    ...
    let embed = new Discord.RichEmbed()
      ...
      .addField(`Role${guild.channels.size == 2 ? "" : "s"}${guild.channels.size <= 44 ? ` [${guild.channels.size - 1}]` : ""}`, `${guild.channels.size <= 44 ? guild.roles.sort((a, b) => {if (a.position < b.position) return 1; if (a.position > b.position) return -1}).map(r => `${r}`).join(' ') : guild.channels.size - 1}`, true)
